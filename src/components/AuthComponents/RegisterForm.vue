...
<template>
  <div class="min-w-xl">
    <div class="sm:mx-auto sm:w-full sm:max-w-sm">
      ...
      <h2 class="text-center text-2xl/9 font-bold tracking-tight text-white">Inscription</h2>
    </div>

    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
      <form action="#" method="POST" class="space-y-6" v-on:submit.prevent="createAccount(form)">
        <div>
          <label for="name" class="block text-sm/6 font-medium text-gray-100">Pseudo</label>
          <div class="mt-2">
            <input v-model="form.name" id="name" type="text" name="name" required class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
          </div>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between">
            <label for="password" class="block text-sm/6 font-medium text-gray-100">Confirmez votre mot de passe</label>
          </div>
          <div class="mt-2">
        ...
        <div>
          <button type="submit" class="flex w-full justify-center rounded-md bg-indigo-500 px-3 py-1.5 text-sm/6 font-semibold text-white hover:bg-indigo-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">S'inscrire</button>
        </div>
      </form>
    </div>
  </div>
